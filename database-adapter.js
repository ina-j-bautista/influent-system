...
export class PostgreSQLAdapter {
    ...
    //######## POST QUERIES #########################
    async getPostsByTimeWindow(startDate, endDate) {
        const query = `
      SELECT 
        post_id,
        user_id,
    ...
    async getPostsByKeywords(keywords, timeWindow) {
    ...
    //######## INTERACTION QUERIES #########################
    async getInteractionsByTimeWindow(startDate, endDate) {
        const query = `
      SELECT 
        from_user,
        to_user,
    ...
    //######## FEATURE QUERIES #########################
    async getEngagementFeatures(fromUser, toUser) {
        const query = `
      SELECT 
        from_user,
        to_user,
    ...
    async getSentimentScores(postIds) {
        if (postIds.length === 0) {
            return [];
        }
        const query = `
    ...
    async getReciprocityStats(userA, userB) {
        const query = `
      SELECT 
        user_a,
        user_b,
    ...
    async getConnectionWeights(userId) {
        const query = `
      SELECT 
        from_user,
        to_user,
    ...
    async getOutgoingConnectionWeights(userId) {
        const query = `
      SELECT 
        from_user,
        to_user,
    ...
    async computeInfluentScore(userId, alpha = 0.33, beta = 0.33, gamma = 0.34) {
        const query = `
